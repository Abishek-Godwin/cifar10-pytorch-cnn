...
def main():
    ...
    train_data = torchvision.datasets.CIFAR10(
        root='D:/data', train=True, download=False, transform=transform_images)

    trainloader = torch.utils.data.DataLoader(
        train_data, batch_size=32, shuffle=True, num_workers=2)

    train_data = torchvision.datasets.CIFAR10(
        root='D:/data', train=True, download=True, transform=transform_images)
    ...
    for epoch in range(20):  
        ...
        for i, data in enumerate(trainloader, 0):
            inputs, labels = data
            # zero the parameter gradients ( after each batch pytorch will accumulate gradients)
            # this is important to do, otherwise the gradients will be added to the previous gradients
            optimizer.zero_grad()
    ...
    torch .save(net.state_dict(), 'trained_cnn.pth')
